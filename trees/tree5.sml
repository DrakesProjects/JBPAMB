...
signature TREE5 =
  sig
    ...
    * no two subtrees of a single node differ in height by more than 1*)
    val avlTreeInvatiantCheck  : tree -> bool
...
functor AddFinalFunctions(T: TREE4) : TREE5 = 
  struct
    ...
    fun wbInvariantCheck(Empty) = true
      | wbInvariantCheck (T1: tree): bool =
      let
        val ALPHA = 0.29
        val BETA = 1.0 - ALPHA
      ...
      in
        (balance(getLChild(T1), getRChild(T1)) andalso
        wbInvariantCheck(getLChild(T1)) andalso 
        wbInvariantCheck(getRChild(T1)))
      end

      fun avlInvaraintCheck(T1: tree): bool =
        let
          fun getH (Empty) = 0
        ...
        in
          ((c1 andalso c2) andalso (avlInvariantCheck(getLChild(T1)) andalso
          avlInvariantCheck(getRChild(T1))))
